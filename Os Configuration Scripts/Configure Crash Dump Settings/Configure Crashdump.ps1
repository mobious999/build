...
If ($crashdumplevel -eq 1 -or $crashdumplevel -eq 2 -or $crashdumplevel -eq 3 -or $crashdumplevel -eq 7) {
  write-host "any 4 of the correct values are there"
}  Else {
...
Try {
  $regkey = "HKLM:\System\CurrentControlSet\Control\CrashControl"
  Set-ItemProperty -Path $regkey -Name "CrashDumpEnabled" -value $crashdumplevel
}
